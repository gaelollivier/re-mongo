...
let decode_reply_doc = str => {
  ...
  let rec decode_doc = (cur, acc) =>
    if (cur >= String.length(str)) {
      ...
      let (len32, _) = decode_int32(str, cur);
      let len = Int32.to_int(len32);
      /*Printf.printf "inside doc len = %d\n" len;*/
      decode_doc(
        cur + len,
        [Bson.decode(String.sub(str, cur, len)), ...acc],
      );
    };
...
let decode_reply = str => {
  ...
  let header_str = String.sub(str, 0, 4 * 4);
  let header = MongoHeader.decode_header(header_str);
  /*print_endline (MongoHeader.to_string header);*/
  let (flags, next) = decode_int32(str, 4 * 4);
  let (cursor, next) = decode_int64(str, next);
  let (from, next) = decode_int32(str, next);
  let (returned, next) = decode_int32(str, next);
  /*print_endline (string_of_int (Int32.to_int returned));*/
  let doc_str = String.sub(str, next, String.length(str) - next);
  let doc_list = decode_reply_doc(doc_str);
  {
    header,
