...
let combine_header_body = (request_id, op, body_buf) => {
  let body_len = Buffer.length(body_buf);
  let whole_buf = Buffer.create(4 * 4 + body_len);
  let header_str =
    encode_header(create_request_header(body_len, request_id, op));
...
let create_insert =
    ((db_name, collection_name), (request_id, flags), insert_doc_list) => {
  let body_buf = Buffer.create(32);
...
let create_delete =
    ((db_name, collection_name), (request_id, flags), selector_doc) => {
  let body_buf =
    create_select_body_buf(
      (db_name, collection_name),
      (request_id, flags),
...
let create_get_more =
    ((db_name, collection_name), (request_id, return), cursor) => {
  let body_buf = Buffer.create(32);
  encode_int32(body_buf, 0l);
  encode_cstring(body_buf, db_name ++ "." ++ collection_name);
  encode_int32(body_buf, return);
...
let create_kill_cursors = (request_id, cursor_list) => {
  let body_buf = Buffer.create(32);
  encode_int32(body_buf, 0l);
  let cursor_buf = Buffer.create(12);
  let rec create_cursor_buf = num =>
    fun
    | [] => num
    | [hd, ...tl] => {
  ...
  let num = create_cursor_buf(0, cursor_list);
