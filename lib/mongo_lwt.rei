...
let insert: (t, list(Bson.t)) => Lwt.t(unit);
...
let update_one: (~upsert: bool=?, t, (Bson.t, Bson.t)) => Lwt.t(unit);
...
let update_all: (~upsert: bool=?, t, (Bson.t, Bson.t)) => Lwt.t(unit);
...
let delete_one: (t, Bson.t) => Lwt.t(unit);
...
let find_q_s: (~skip: int=?, t, Bson.t, Bson.t) => Lwt.t(MongoReply.t);
...
let find_q_s_one: (~skip: int=?, t, Bson.t, Bson.t) => Lwt.t(MongoReply.t);
...
let find_q_s_of_num:
  (~skip: int=?, t, Bson.t, Bson.t, int) => Lwt.t(MongoReply.t);

/** counts the number of documents in a collection */

let count: (~skip: int=?, ~limit: int=?, ~query: Bson.t=?, t) => Lwt.t(int);
...
let get_more_of_num: (t, int64, int) => Lwt.t(MongoReply.t);
...
let get_indexes: t => Lwt.t(MongoReply.t);

/** ensure an index */

let ensure_index: (t, Bson.t, list(index_option)) => Lwt.t(unit);

/** ensure an index (helper) */

let ensure_simple_index:
  (~options: list(index_option)=?, t, string) => Lwt.t(unit);

/** ensure multi-fields index (helper) */

let ensure_multi_simple_index:
  (~options: list(index_option)=?, t, list(string)) => Lwt.t(unit);

/** drop a index */

let drop_index: (t, string) => Lwt.t(MongoReply.t);

/** drop all index of a collection */

let drop_all_index: t => Lwt.t(MongoReply.t);
...
let drop_collection: t => Lwt.t(MongoReply.t);

/** drops a database, deleting the associated data files */

let drop_database: t => Lwt.t(MongoReply.t);
